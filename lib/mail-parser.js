            ...
            if (node.textContent) {
                if (node.contentType === 'text/plain') {
                    ...
                    if ((!alternative && this.hasText) || (node.root && !this.hasText)) {
                        ...
                        } else if (!this.options.skipHtmlToText) {
                            try {
    ...
    processChunk(data, done) {
        ...
        switch (data.type) {
            case 'node': {
                ...
                if (node === this.tree) {
                    ['references', 'date', 'to', 'from', 'to', 'cc', 'bcc', 'message-id', 'in-reply-to', 'reply-to'].forEach(key => {
                        if (node.headers.has(key)) {
                            this[key.replace(/-([a-z])/g, (m, c) => c.toUpperCase())] = node.headers.get(key);
                        }
                    });
                    if (node.headers.has('subject')) {
                        let subject = node.headers.get('subject')
                        if (shouldDecode(charset)) {
                            try {
                                const iconv = createIconv(charset);
                                subject = iconv.convert(node.headers.get('subject')).toString();
                                node.headers.set('subject', subject);
                            } catch (E) {
                                // Skip decoding
                            }
                        }
                ...
                if (node.isAttachment) {
                    ...
                    if (shouldDecode(charset)) {
                        try {
                            stream = createIconv(charset).stream();
                        } catch (E) {
                            // Skip decoding on error
                        }
                    }
                    if (stream) {
                        ...
                        node.contentStream.pipe(stream);
                        node.contentStream = stream;
                    } else {
...
function shouldDecode(charset) {
    return !['ascii', 'usascii', 'utf8'].includes(
        charset
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '')
            .trim()
...
function createIconv (charset) {
    if (iconvLite.encodingExists(charset)) {
        return {
            stream () {
                return iconvLite.decodeStream(charset);
            },
            convert (str) {
                return iconvLite.decode(Buffer.from(str, 'binary'), charset);
            }
        }
    } else {
        return {
            stream () {
                return new Iconv(charset, 'utf8');
            },
            convert (str) {
                const iconv = new Iconv(charset, 'utf8');
